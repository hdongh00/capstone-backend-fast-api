...
CHAT_COLLECTION = client["chatbot"]["messages"]
SUMMARY_COLLECTION = client["chatbot"]["summary_records"]

model = ChatGoogleGenerativeAI(model="gemini-2.5-flash", google_api_key=GOOGLE_API_KEY, temperature=0.3)

try:
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    tokenizer = AutoTokenizer.from_pretrained(MODEL_PATH if os.path.exists(MODEL_PATH) else "beomi/kcbert-base")
    emotion_model = AutoModelForSequenceClassification.from_pretrained(
        MODEL_PATH if os.path.exists(MODEL_PATH) else "beomi/kcbert-base", num_labels=3)
...
LABELS = ["부정", "중립", "긍정"]
EMOTION_SCALE = {
    1: "매우 부정적", 2: "부정적", 3: "다소 부정적", 4: "약간 부정적",
    5: "중립적", 6: "약간 긍정적", 7: "긍정적", 8: "매우 긍정적"
}

prompt = ChatPromptTemplate.from_messages([
    ("system",
     "너는 사용자의 감정을 깊게 공감하는 정신 건강 상담 챗봇이고, 너는 무조건 사용자에게 오늘 하루에 있었던 일에 대하여 지속적으로 질문하고 공감해줘야한다. 또한 절대로 전에 했던 history 대화에 있는 질문에 답변해선 안된다."),
...
summary_prompt = ChatPromptTemplate.from_messages([
    ("system",
     "너는 이 history 대화를 모두 요약하여 철저히 사용자의 시선에서 한 편의 일기를 써야하는 일기 마스터이다. 어투는 오늘은 ~ 했다 또는 ~가 있었다 등 과거형으로 집필해야하며, 안좋은 내용이 있더라도 객관적으로 작성해야한다."),
    MessagesPlaceholder("history"),
    ("human", "이 history를 기반으로 모두 대화를 요약하고, 내 관점에서 일기를 작성해줘. 일기를 작성해 달라고 이야기 한 것은 빼줘.")
])
...
async def summary(request: Request, authorization: str = Header(...)):
    ...
    # SUMMARY_COLLECTION.insert_one(summary_record)
...
async def analyze(request: Request, authorization: str = Header(...)):
    ...
    if not user_utterances:
        return {"message": "대화가 없음", "result": None}
    ...
    avg_scale_score = total_scale_score / len(user_utterances) if user_utterances else 0
